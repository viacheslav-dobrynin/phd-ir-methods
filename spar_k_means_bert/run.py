...
def build_hnsw_index():
    ...
    for token, doc_ids in tqdm.tqdm(
        ...
        gmm = sklearn.mixture.GaussianMixture(
            n_components=args.kmeans_n_clusters
            if len(embs) > args.kmeans_n_clusters
            else len(embs),
            init_params="k-means++",
            covariance_type="diag",  # обычно «diag» работает стабильно; можно «full» при достаточном числе примеров
            max_iter=100,  # по желанию – аналог n_init у KMeans не нужен
            random_state=args.seed if hasattr(args, "seed") else None,
        )
        gmm.fit(embs)
        # centroid’ы берём из gmm.means_
        for i, centroid in enumerate(gmm.means_):
            hnsw_index.add(np.array([centroid]))
