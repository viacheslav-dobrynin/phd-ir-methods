...
def build_hnsw_index():
    ...
    for token, doc_ids in tqdm.tqdm(
        ...
        birch = sklearn.cluster.Birch(
            threshold=0.5,  # радиус CF-подкластера; уменьшите, если хочется > кластеров
            branching_factor=50,  # сколько «дочерних» CF-узлов на уровень
            n_clusters=n_clusters,
        )  # финальное число кластеров (доклеивает Agglo)
        birch.fit(embs)
        for i, centroid in enumerate(birch.subcluster_centers_):
            hnsw_index.add(np.array([centroid]))
