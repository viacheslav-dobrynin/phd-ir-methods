...
def build_hnsw_index():
    ...
    for token, doc_ids in tqdm.tqdm(
        ...
        if embs_n == 0:
            continue
        if embs_n < 2:
            centroid = embs.mean(axis=0)  # единственный вектор
            hnsw_index.add(np.array([centroid]))
            faiss_idx_to_token[hnsw_index.ntotal - 1] = f"{token}_0"
            continue
        n_clusters = (
            args.kmeans_n_clusters if len(embs) > args.kmeans_n_clusters else len(embs)
        )
        connectivity = sklearn.neighbors.kneighbors_graph(
            embs,
            n_neighbors=30 if len(embs) > 30 else len(embs) - 1,
            include_self=False,
            mode="distance",
            n_jobs=-1,
        )
        # Agglomerative (по умолчанию linkage='ward', affinity='euclidean')
        agg = sklearn.cluster.AgglomerativeClustering(
            n_clusters=n_clusters, connectivity=connectivity
        )
        labels = agg.fit_predict(embs)  # возвращает метку для каждого вектора

        # считаем центроиды и добавляем их в индекс
        for i in range(n_clusters):
            cluster_vecs = embs[labels == i]
            centroid = cluster_vecs.mean(axis=0)  # np.ndarray размером (d,)
