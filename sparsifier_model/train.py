...
def get_trainer(config: Config, logger):
    if config.devices:
        return L.Trainer(max_epochs=config.epochs,
...
def train(config: Config,
    ...
    doc_embs_kmeans_centroids = get_kmeans_centroids(config=config, dataloader=dataloader)

    model = SparserModel(config=config,
    ...
    try:
        ...
        wandb_logger = L.loggers.WandbLogger(project=config.project, log_model=True, name=model_name, id=model_name)
